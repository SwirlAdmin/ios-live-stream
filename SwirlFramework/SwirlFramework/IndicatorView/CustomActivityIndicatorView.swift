...
public class CustomActivityIndicator: NSObject {
    ...
    fileprivate static let shared: CustomActivityIndicator = {
        let instance = CustomActivityIndicator()
    ...
    private override init() {
        ...
        if let window = delegate?.window, let innerWindow = window {
            self.window = innerWindow
        } else {
    ...
    fileprivate func configureActivityIndicator(withStatusMessage message: String, isUserInteractionEnabled userInteractionStatus: Bool) {
        ...
        if spinnerContainerView != nil {
            ...
            CustomActivityIndicatorStyle.createSpinner(in: viewLayer, size: animationRectsize, color: spinnerColor, style: activityIndicatorStyle)
        }
